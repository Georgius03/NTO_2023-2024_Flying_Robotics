            ...
            if 400 <= cv2.contourArea(cnt1) <= 16000:
                ...
                for cnt2 in contours2:
                    if 100 <= cv2.contourArea(cnt2) < 1600:
                        ...
                        cropped_image21 = cropped_image2
                        text = pytesseract.image_to_string(cropped_image21)  # получение текста из картинки
                        self.check_and_add(self.text_replace(text))
                        # print(text_replace(text))

                        cropped_image22 = cv2.rotate(cropped_image2, cv2.ROTATE_90_CLOCKWISE)
                        text = pytesseract.image_to_string(cropped_image22)  # получение текста из картинки
                        self.check_and_add(self.text_replace(text))
                        # print(text_replace(text))

                        cropped_image23 = cv2.rotate(cropped_image2, cv2.ROTATE_180)
                        text = pytesseract.image_to_string(cropped_image23)  # получение текста из картинки
                        self.check_and_add(self.text_replace(text))
                        # print(text_replace(text))

                        cropped_image24 = cv2.rotate(cropped_image2, cv2.ROTATE_90_COUNTERCLOCKWISE)
                        text = pytesseract.image_to_string(cropped_image24)  # получение текста из картинки
                        self.check_and_add(self.text_replace(text))
                        # print(text_replace(text))
